...
- ⚠️ **모델에게 뽑기와 변환을 함께 시키지 않는다**(2026-08-12 실측). 맥락이 필요한 것만 모델이 하고, 맥락 없이 글자만 보면 되는 변환은 **`pipeline/normalize.py`** 가 한다.
  ...
  - ⏳ **지역을 모델로 옮기기로 했다**(2026-08-16 결정 · **아직 코드는 `place_of`다** · SPEC §5.5b). `location` → `region`+`city`는 게시판 지역 칸에서 100%였지만, `안동시`처럼 **도시만 적힌** 공고(11%)와 `전남광주통합특별시 북구 오치동`(그 표기 12건 중 **4건**이 광주인데 JEONNAM으로 오판)은 **글자가 아니라 지리 지식**이 필요하다. 표로 담으려면 동 이름 3,500줄이 된다 → 모델이 `region`·`city`·`address`를 근거와 함께 내고 **코드는 검산만** 한다.
