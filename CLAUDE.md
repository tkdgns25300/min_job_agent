...
| **parse** (`sources/adapters/`) | 목록 행·상세 본문 추출 | **게시판 1곳 = 파일 1개**(31개) |

→ 신규 소스는 **config 한 칸 + 어댑터 파일 1개**다.

⚠️ **CMS 계열별로 묶지 않는다**(운영자 결정 2026-08-04). 그누보드·Konnect·eGov를 쓰는 곳이 몇 개씩 있지만 **공통 부모(계열 base 클래스)를 만들지 않는다** — 게시판은 각자 따로 바뀌는데 코드가 묶여 있으면 한 곳을 고치다 나머지를 깨고, 파싱을 이해하려면 파일 두 개를 오가야 한다. **31곳을 하나씩 독립으로 관리**하는 것이 유지보수에 유리하다.
