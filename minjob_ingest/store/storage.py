...
**왜 보관하나** — 검수 554건 중 **96%가 포스터 공고**다(SPEC §5.7). 검수 화면이 원본 게시판
URL을 그대로 `<img src>`에 걸면 최소 5~7곳이 안 뜬다: `http_only` 2곳(https 페이지에서 mixed
content 차단) · `insecure_tls` 3곳(인증서 오류) · `needs_session` 2곳(쿠키 필요) + 열거할 수
없는 referer 차단. 게다가 게시판이 글을 지우면 판정을 되짚을 증거가 사라진다.
