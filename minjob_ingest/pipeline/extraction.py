...
- role: GENERAL일 때 무슨 일인지 **원문 표현을 살려 20자 안으로**. 정해진 목록이 아니다 —
  ...
  MINISTRY만 있으면 null.
- deadline: 마감일을 **원문 표기 그대로**(`2026-08-31`·`2026년 8월 31일까지`·`8월 31일까지`).
  ⚠️ **연도가 없어도 그대로 옮긴다** — 연도는 코드가 게시일에서 붙인다. 지어내지 말고
  원문 글자만 준다. 기간이면 통째로(`8월 12일 ~ 9월 10일`).
...
def has_prompt_evidence(record: SourceData) -> bool:
    ...
    공고였는데 "빈 입력"으로 조용히 버려졌다(같은 비율이면 2개월에 50건이 넘는다).
