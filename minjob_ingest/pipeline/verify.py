...
_RECRUITING_WORDS: Final = (
    ...
    "모십",
    "찾습",
    # ⚠️ 모집 목록의 **머리말**이다(`1. 대상 : 담임목사` — KTS 양식 4건). 동사가 없어도 뽑는
    #    자리라는 뜻이다. `분야`는 넣지 않는다 — `사역분야: 담임목사 사역 협력`처럼 담임을
    #    **돕는** 자리를 담임 청빙으로 읽는 줄이 6건이다(실측 2026-08-15).
    "대상",
)
...
class VerifyReport:
    ...
    #: ⚠️ **이 숫자는 `confidence`를 낮추지 않는다**(2026-08-17 결정). 실측 초안 132건 중 53건
    #: (40%)이 `unchecked>0`이고 자동 승인될 108건 중 37건(34%)이 그렇다 — 등급을 내리면
    #: 검수량이 24건에서 61건으로 늘어난다. 어긋난 칸은 프롬프트가 **조립을 시킨** 곳에
    #: 몰려 있고(`benefit_note` 16 · `headcount` 10 · `required_docs`·`process_steps` 각 6),
    #: 손으로 본 23개 중 지어낸 것은 0개였다. **프롬프트 수정의 신호로만 쓴다**(SPEC §5.7).
    unchecked: int = 0
