...
- [x] **CSU 첨부 다운로드 URL을 고쳤다** ✅ 2026-08-23 — 루트가 `/upload/`(추측)가 아니라 `/api/file/get?path=`다. 첨부가 **처음부터 전부 404**였는데, 저장된 CSU 포스터 10건이 모두 **본문 인라인 그림**이어서 수치가 멀쩡해 오래 드러나지 않았다. ⚠️ 교훈: 한 게시판에 그림 경로가 둘이면 **둘을 따로 확인한다** — 한쪽 성공이 다른 쪽 실패를 가린다. 테스트가 두 URL을 한 묶음으로 못 박는다(인라인이 정답의 증거). ⚠️ **이미 저장된 6건은 낫지 않는다** — `source_data`가 write-once라 옛 주소가 남고 재구조화가 그것을 읽는다(SPEC §5.7 ⚠️⚠️). 2개월 전량을 **새로 수집**하면 처음부터 옳게 들어온다
