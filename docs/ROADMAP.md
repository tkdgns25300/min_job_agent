...
- [x] **길이 상한** — `title`·`church_name`은 **200자**, 목록 항목은 300자로 막는다(원문이 그 칸으로 흘러 공개되는 것을 막는다). ⚠️ **`description`은 상한을 두지 않는다**(운영자 결정 2026-08-11) — 상한이 있으면 조금 넘긴 공고가 실패하고 재시도 상한을 넘겨 사라진다. 요약 강제는 프롬프트, 최종 방어선은 운영자 검수다
...
- [ ] **`description`의 정의를 프롬프트에 넣는다** — "**표 밖에서 알아야 하는 것**"이다: 교회가 어떤 곳인가(`말씀중심·기도중심·선교중심`·`전원 속 도심교회`·`설립 51년`) · 이 자리가 어떤 자리인가 · 표에 안 들어가는 조건. **사실은 반복하지 않는다**(부서·인원·자격·서류·마감·처우·연락처는 각자 칸이 있다). ⚠️ 지금 이 문장이 없어서 1단계 실측에서 모델이 사실을 요약에 반복하고 **교회 소개 2문단을 통째로 버렸다**
- [ ] **한 글에 자리가 여러 개일 때 — 지금은 나누지 않는다**(운영자 결정 2026-08-11). 실측: 직분을 2개 이상 적은 공고에서 **Ⓑ 한 자리·자격만 열어둠 826건 : Ⓐ 진짜 여러 자리 128건(+애매 93건)** — 6.5:1로 Ⓑ가 압도적이라 "직분이 여럿이면 분리"는 **없는 자리 826건을 만든다**.
...
- [ ] **필드마다 세 갈래로 판단한다**(min_job 검수 반영 2026-08-11). ⚠️ **자리가 여러 개라고 필드를 비우는 것이 아니다** — 비우는 이유는 둘이고 서로 다르다:
  ```
  그 칸의 값이 공고에 적혀 있나?
...
- [ ] **`job_kind`도 배열**(min_job 검수 반영 2026-08-11) — `② 교육전도사 2명 ③ 관리직원 1명`처럼 사역직·일반직이 섞인 공고는 **지금 스키마로 표현 자체가 불가능**하다(CHECK가 `position`과 `role` 동시 보유를 금지 → 반드시 절반을 버린다). 건수는 적지만(키워드 동시 출현 492건 중 자격문구를 걷어내면 121건, 실제로는 그보다 적다) `position`을 배열로 바꾼 이유와 같은 문제다. min_job 비용도 타입 1곳 + mock뿐(일반직 UI 미구현).
  - ⚠️⚠️ **CHECK는 `COALESCE(cardinality(...))`로 써야 한다**(2026-08-11 실 Postgres 8케이스 검증). `array_length(x,1) > 0`은 **빈 배열을 통과시킨다** — `array_length('{}',1)`이 `NULL`이고 Postgres CHECK는 `TRUE` **또는 `NULL`**이면 통과하기 때문이다(`FALSE`만 거부). `cardinality('{}')`는 `0`이라 안전하고, 배열 자체가 NULL일 수 있어 `COALESCE`가 한 겹 더 필요하다.
    ...
    ```
- [ ] **`job_kind` ↔ `position`/`role` 불변식을 `ReviewData`에도 넣는다** — ⚠️ 지금 우리 레코드는 이걸 검사하지 않아 min_job DB만 막고 있다. 그러면 어긋난 초안이 **승격 시점에야** 터진다. 저장 전에 걸려야 그 공고 하나만 실패하고 배치가 계속된다(다른 불변식과 같은 취급)
...
- [ ] **뽑지 않기로 한 것** — 담임목사 이름(`jobs` 필드가 아니고 `source_data`에 보존됨) · 교회 홈페이지 URL(담을 칸이 없고 `raw_html` 1,370건(43%)에 남아 **재수집 없이** 꺼낼 수 있다) · 날짜가 아닌 마감 표현(`충원시까지` → `deadline=null`. min_job이 3개월 지난 공고를 숨긴다)

- [ ] **출력 JSON 계약(34필드)** — `review_data`와 1:1로 확장 + enum 밖 값 방어적 정규화
...
- [ ] **연속 저장 실패 시 중단** — ⚠️ 글 단위 격리는 되지만, `review_data.json`이 통째로 깨지면 공고마다 **Gemini를 부른 뒤** 저장이 실패한다 → `--all`이 3,000회 과금하고 아무것도 저장하지 못한다. 연속 N회 저장 실패면 실행을 멈춘다
- [ ] **되돌리기 스크립트** — `scripts/reset_structure.py`(로컬 JSON 한정 · CLI 명령이 아니다). ⚠️ **전량 실행 전에** 있어야 한다(위 ⚠️⚠️)
- [ ] 소량(`--dry-run --limit 20 --source KEY`) 검증 → 프롬프트 조정 → ⚠️ **전량은 운영자가 실행**(유료 API)
