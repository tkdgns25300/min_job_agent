...
- [x] **데일리(증분) 모드** ✅ 2026-08-24 — `daily`가 네 단계를 잇는다. 창 = **마지막 성공 실행 − 1일**(7일 상한 · 넘으면 경고) · `mode=DAILY` · 유료 상한 500건 · **판정 미완이면 `publish` 건너뜀**(`StoreError`가 그 경계와 일치한다). 창 계산은 순수 함수(`collect.daily_window`)라 무료로 테스트된다
  - ⚠️ **실패한 실행은 창의 기준이 되지 못한다** — 그걸 기준으로 삼으면 못 가져온 글이 다음 실행의 창 밖으로 밀려 유실된다. 실패가 이어지면 창이 저절로 넓어진다
  - ⚠️ **종료코드를 판정으로 쓰지 않는다** — 게시판 한 곳이 죽는 것은 정상 상황이다. 판정은 `status`가 한다
