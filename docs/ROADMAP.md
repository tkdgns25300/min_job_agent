...
- [ ] **`confidence` 3단계 규칙**(운영자 결정 2026-08-11) — SPEC §6이 "검수 우선순위는 교단보다 지역"이라 한 것을 그대로 쓴다. `high` 승격 6개 + `region` 있음 / **`medium` 승격 6개는 다 있는데 `region`이 없음**(공개는 되지만 min_job 지역 필터에서 탈락해 사실상 안 보인다 → 지역만 채우면 된다) / `low` 6개 중 하나라도 없음 · 게이트1 `UNCERTAIN` · 이미지를 못 읽음. ⚠️ 이 규칙이 없으면 `medium`을 만드는 경로가 없어 세 값 중 둘만 쓰인다
