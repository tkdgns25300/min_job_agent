...
- [x] **칸 두 개** — `dedup_key`(같은 자리 후보) + `dedup_state`(ALONE/MASTER/DUPLICATE/SEPARATE/UNCERTAIN). ⚠️ 판정 결과를 키 문자열에 우겨넣지 않는다(운영자 지적 2026-08-17) · 레코드 불변식이 `DUPLICATE` ⇔ `reject_reason=DUPLICATE`를 한 몸으로 묶는다
...
- [x] **연락처로 가른다**(제목 아님) — 실측 19묶음에서 연락처가 답을 냈다. 제목 규칙은 이득 0·비용 1이라 뺐다
