...
> ⚠️ **1단계는 이미지 공고를 건드리지 않는다.** 프롬프트가 텍스트만 보내므로, 이미지가 있는 공고(실측 **237건 · 7.4%** · 그중 116건은 본문이 아예 없다)는 `DEFERRED`로 두고 **판정도 시도 횟수도 남기지 않는다**. 그대로 판정하면 포스터 공고가 "내용 없음"으로 읽혀 게이트1 `NO`가 되고 `structured_at`이 찍혀 **2단계가 다시 볼 수 없다**. 2단계가 멀티모달을 붙이면 `structure.waits_for_images`가 사라지고 같은 행이 정상 처리된다.
...
- [x] **길이 상한** — `description`(1,000자)뿐 아니라 `title`·`church_name`도 **200자**로 막는다. ⚠️ 요약만 막으면 원문이 `title`로 흘러 공개된다(가드레일 #3)
- [x] **맥락은 다 준다** — `author`·`senior_pastor` 같은 실명도 프롬프트에 넣는다(운영자 결정 2026-08-10). 가드레일 #4는 "제3자 개인정보를 **추출**하지 않는다"이지 모델에 맥락으로 주지 말라는 뜻이 아니고, 맥락이 적을수록 오추출이 는다. 막는 자리는 **출력 계약**이다 — 1단계 4필드에 사람 이름 칸이 없다
...
- [ ] **한 글에 자리가 여러 개일 때 — 지금은 나누지 않는다**(운영자 결정 2026-08-11). 실측: 직분을 2개 이상 적은 공고에서 **Ⓑ 한 자리·자격만 열어둠 826건 : Ⓐ 진짜 여러 자리 128건(+애매 93건)** — 6.5:1로 Ⓑ가 압도적이라 "직분이 여럿이면 분리"는 **없는 자리 826건을 만든다**(가드레일 #8보다 나쁘다 — 중복이 남는 것과 달리 되돌릴 수 없다).
...
- [ ] **직분은 여러 개 붙인다**(운영자 결정 2026-08-11) — 실측 **634건(19.9%)** 이 한 공고에 직분을 여러 개 적는다. 두 경우 모두 **행을 쪼개지 않고 직분만 여러 개** 단다: ⓐ 한 자리인데 자격이 여러 직분(`전임사역자(전도사, 강도사, 목사)`) ⓑ 여러 자리(`1.부목사 2.교육목사 3.여전도사`). 대표 1개만 넣으면 나머지 직분으로 검색한 지원자에게 **안 보인다**. 행 분리는 `UNIQUE(source_data_id)`를 깨고 검수를 4배로 만든다
  - ⚠️ **min_job 스키마 변경이 선행돼야 공개된다**(가드레일 #9 — 운영자 작업): `jobs.position` `text`→`text[]` · 목록 필터 인덱스를 GIN으로 · 필터 쿼리 `= 'X'`→`@> ARRAY['X']` · ⚠️ `ChurchVerification.applicant.position`은 **개인 직분이라 배열화 대상이 아니다** · `role`(일반직)은 자유 텍스트라 단일 유지 · 목록 카드는 축약 필요(`부목사 외 2 · 유초등부`)
  ...
  - ⚠️ **dedup에 영향**: `dedup_key`의 `position` 자리는 **정렬해 이어붙인 값**이 된다. 끌어올림이 직분을 하나 빼면 키가 달라져 **병합되지 않는다**(중복이 남는다) — 다른 교회를 합치는 것보다 낫다(가드레일 #8)
...
- [ ] **출력 JSON 계약(33필드)** — `review_data`와 1:1로 확장 + enum 밖 값 방어적 정규화
...
- [x] **테스트 seam** — 파이프라인은 Gemini를 **좁은 Protocol**로 받는다(구상 `GeminiClient`를 직접 받으면 테스트가 네트워크를 타야 한다 · 가드레일 #7). fake로 5가지: 정상 1건 · 게이트1 `NO` · 빈 공고(호출 0회) · 실패(`structured_at` 없음 + `attempts` +1) · **저장 순서**
