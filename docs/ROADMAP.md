...
> | **3단계** 마감·전량 | 규칙 판정(~~교단~~ ~~이단~~ ✅ 2026-08-16 · `dedup_key`·`confidence`) + **되돌리기 스크립트** ✅ + 진행 표시 | 규칙 테스트(무료) → 운영자가 `--all` | 규칙 0회 · 전량 3,188회 |
>
...
- [x] **2단계 — 자동 승인 규칙** ✅ 2026-08-17 · 실측 자동 승인 77%·사람이 볼 것 17%(≈ 554건)·자동 거절 1% —(`confidence` · SPEC §5.7). ⚠️ **2026-08-11 결정을 개정한다** — 옛 규칙은 "high=승격 6칸+region / medium=region 없음 / low=나머지"였고 **모두 PENDING**이었다. 새 규칙은 **검수를 사람이 봐야 답이 나오는 것만 남기고**(포스터·그림 실패·승격칸 빔 ≈ 20%) 나머지를 `APPROVED`로 만든다
