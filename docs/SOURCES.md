...
| 웨스트민스터신대원(WGST) 교역자청빙 | `www.wgst.ac.kr/wgst_renew/board/board.asp?key=6131` | 공개 | ~1,521건 · 2026 최신 미확인 | 🔸 | **http 전용** · 검색상 **기하성(순복음)이 상위 교단** 정황 → 초교파 여부·현 교단 확인 + 2026 활성 확인 후 채택 |
| 에스라성경대학원 초빙게시판 | `www.ezra.ac.kr/main/sub.html?boardID=www48` | 공개 | 저빈도(2026-03) | 🔸 | 168건 · 보조 |
...
| 후보 | 힌트 | 상태 |
|---|---|---|
| ↑ 채택됨(§1 초교파) | 횃불트리니티·아신대 ✅ · WGST·에스라 🔸 · CTS 🔒 | 이번 실측으로 확정 |
| 서울신학교 사역게시판 | `stc68.net/board` | ⚠️ **stc68=서울신학교(동작구·소규모·별개)** ≠ 서울신학대(기성, `stu.ac.kr`) — Fable가 '기성 대표'로 오인. 활성 주장 있으나 SSL 오류로 미확정 → 확인 후 결정 (기성 대표 stu 게시판 유무 별도 확인) |
| 하나님의성회 교역자청빙(공개) | `agkdc.org/bbs/board.php?bo_table=07_6` | **신규** — 순복음(AG) **공개** 청빙 게시판(로그인벽 agk와 달리 열람 가능·실제 글 확인) → 최신성 확인 후 순복음 보조 채택 검토 |
| 나사렛성결회 목회자청빙 | `na.or.kr/ccall` | 실재(112건, 07-06) 저물량 → ETC, 후순위 |
| 순복음통합 총회 | `agkr.org` | 순복음통합 공식 사이트 실재 · 자체 청빙판 미확인(공개 청빙은 위 agkdc) |
| 서울성경신대원(SU) | `sb.ac.kr?boardID=www85` | 예장 순장→ETC · 저물량(~34) → 보조·후순위 |
| 대전신학대 | `daejeon.ac.kr` | 게시판 미발견 → **드롭**(과거 '없음' 확인) |
...
## 7. 크롤 우선순위
- **Tier 1 (공개·활발·즉시)**: 대신대·칼빈대·광신대·총신대(합동) · 영남·장신·호남·부산장신·PCK·서울장신·한국기독공보·**한일장신**(통합) · 백석대대학원·백석총회(백석) · 감신대·협성대(감리) · 한세대·순복음대학원대(순복음) · 침신대·침례총회(침례) · 기성·예성(성결) · KTS(고신) · 합신대(합신) · 기장·KAICAM(기타) · **횃불트리니티·아신대(초교파, 모드 B)**
- **Tier 2 (조건부·보완)**: 예장합동총회(저조) · 목원대(둔화) · 순복음총회신학교·고신대신학과(청빙 필터) · 백석대신총회(유동) · WGST·에스라(초교파 조건부)
- **로그인 티어 (게이트·계정 후)**: KMC · AGK · CTS(상세 로그인) (기독신문은 전면 로그인벽이라 **제외 유력**)
- **확인 필요 (§3)**: agkdc(공개 순복음) · 서울신학교(stc68) · 나사렛 · SU · agkr
- **초기 제외 (참고용)**: 청빙넷 · 제이웹 · cjob · 갓피플 · 더미션 · 기독일보
