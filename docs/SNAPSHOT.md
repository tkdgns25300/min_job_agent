...
#### ② 중복 · 끌어올림 — ⚠️ **잠정. 더 논의해야 한다** (2026-08-06 3차)

```
잠정 키:  교회명(정규화) + 제목(정규화)      창: 3개월
          같으면  →  1건만 · posted_at 최신
...
| **교회명 + 제목** ← 잠정 채택 | **1,854건** | **41.8%** | 안전(제목 다르면 안 합침) · 교회명 못 뽑는 114건은 못 묶음 |
...
| 중복 키 | **교회명 + 정규화 제목**, 3개월 창 — ⚠️ **잠정**(위 ② 논의 계속) |
...
**키와 창은 잠정이다.** 위 ② 절의 "아직 논의가 남은 지점" 5개를 정리해야 한다:
교회명 못 뽑는 114건 · 제목이 크게 다른 같은 자리 · 마감일 활용 여부 · 데일리 수집 시 창의 의미 ·
교차게시. 구조화를 돌려 `position`·`department`가 실제로 얼마나 정확한지 본 뒤 정밀화하는 것이
순서일 수 있다(그때 min_job `repostKey`와 같은 키로 맞출 수 있다).
