...
**공식 크롤 후보 34** = 채택-공개 26(hanil ✅ 상향 포함) + 로그인티어 3(kidok 제외 유력) + 조건부 5. **+ 초교파 축(신규 검토): 횃불트리니티·아신대 등 — §6·§7 결정 대기.** (CROSS 상업 참고 · 제외 다수)
...
**결정 대기 (신뢰성 직결 — 우선):**
- [ ] **교단 확정 방법** ⭐ — 게시판 default 교단은 **힌트일 뿐**(그 게시판 공고가 다 그 교단은 아님 — 교차게시·초교파·혼재). 신뢰성 우선 방향 = **① 노회/연회명 → 교단 lookup 테이블(결정적) ② 교단 공식 명부 대조(정본) ③ AI는 신호 "추출"만, 매핑은 테이블이(LLM 환각 방지)**. `denomination` + `source`(nohoe/stated/registry/board_default) + `evidence`(근거 인용) 3필드. **근거 없으면 "교단 미상"으로 두고 운영자 게이트**(추측을 사실로 게재 안 함). → CONTRACT §2 명문화 예정.
- [ ] **초교파 축 편입 여부** — 횃불트리니티(즉시 추천)·아신대(정정 후 채택 후보)를 **모드 B(공고별 교단 감지)**로 넣을지. WGST/CTS 조건부(http·로그인), 에스라/SU 보조.
- [ ] **커버리지 정책** — "공식 게시판만"은 실 청빙 물량의 ~25~55%(교단 편차 큼). 물량 우선이면 상업 애그리게이터(청빙넷·cjob·갓피플) 포함 재검토.
