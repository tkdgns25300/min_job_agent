...
## 2026-08-06 (2차) — ★ Phase 1-2 구조화 설계 확정 (여기서 이어받는다)

> **집에서 이어받을 때 이 절만 읽으면 된다.** 아래 두 주제가 논의의 전부이고, 미결은 2개뿐이다.
...
```
키:  연락처 + 직분 + 부서            ← 교회를 몰라도 판정된다 (교회 해결이 선행 조건이 아님)
     마감일이 같다                → 같은 공고 (마감일 있는 것 71%)
     마감일 없고 직전과 간격 ≤ N일   → 끌어올림 → 한 행 · posted_at 갱신
     간격 > N일                  → 재공고 → 새 행으로 보존 (가드레일 #8)
```
...
#### ③ 이단 — **플래그만. 자동 거부 안 함**(운영자 결정 2026-08-06)

```
AI가 교회명 추출 → config/heresy-ref.json 정확 일치 대조 → heresy_flag + heresy_evidence
운영자가 검수에서 판단
```
...
| # | 결정할 것 | 선택지 |
|---|---|---|
| **①** | **끌어올림 간격 N일** | 14일(검수 811건) / 30일(709건) / 묶지 않음. ⚠️ 마감일 있는 71%는 마감일로 판정되므로 **나머지 29%에만** 적용된다 |
| **②** | **min_job 스키마 변경 3개** | `jobs.church_id` NOT NULL 해제 · `jobs.church_name text NOT NULL` 추가 · `jobs.region text NULL (CHECK)` 추가 |

②를 하지 않으면 교회 행을 우리가 만들어야 하고, 그러면 **잘못된 병합 위험이 돌아온다**(위 67개
사각지대). ①만 정하면 구조화 출력 스키마 설계에 바로 들어갈 수 있다.
