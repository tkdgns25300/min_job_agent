...
⚠️ 이 방침은 가드레일 #5("이단은 플래그만 · 최종 판단은 사람")보다 강하다 — **운영자 결정으로
...
운영자 요구는 "최소한으로 검수"였고, 가드레일 #2는 "**크롤러가** 자동 공개하지 않는다"이지
...
6. 소량(`--limit 20`) 검증 → 프롬프트 다듬기 → **전량은 운영자가 실행**(유료 API · 가드레일 #10)
