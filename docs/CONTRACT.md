...
> ⚠️ **부분일치 충돌 → 걸린 표현이 다른 표현 **안에 들어 있으면** 버린다**(포함 관계): `합동신학`→HAPSIN · `합동정통`→BAEKSEOK · `합동보수`→ETC 가 걸리면 그 안의 `합동`(HAPDONG)은 근거가 아니다(예: "예장합신"을 HAPDONG으로 오인 금지).
>
> ⚠️ **"긴 것부터 시도"가 아니다**(2026-08-15 변경 · 구현 `pipeline/denomination.py`). 길이로 고르면 `예장합동 … 합동신학대학원 졸업자 우대`에서 **지원자의 학교**(합신)가 그 교회의 교단이 된다. 포함 관계는 표의 줄 순서에 기대지 않아, 줄 하나를 옮겨 적어도 답이 변하지 않는다. 두 표현이 서로를 품지 않으면(`나사렛` vs `성결`) **확정하지 않는다** — 그래서 `대한기독교나사렛성결회`는 `나사렛성결`을 한 줄로 적는다(`NAZARENE` 3건 전부가 이 표기).
>
> ⚠️ **"그 외 전부 → ETC"는 구현하지 않는다.** 못 알아본 글자는 `UNKNOWN`이다 — `ETC`는 "그 외 **교단**"이라는 주장이라 `아래 참조` 같은 값이 그리로 가면 거짓이 된다(SPEC §5.3의 `UNKNOWN` vs `ETC`). 같은 이유로 `초교파`도 맵에 없다: 소속이 **없다**고 적은 값이라 `NULL`(미상 또는 무소속)이 답이다.
