...
**교단 신호 (강 → 약, 위에서부터 시도):**
1. **노회/연회 이름** — 공고에 노회명(장로교)·연회명(감리교)이 있으면 §2b 표로 조회. **유일 매칭 → 확정(high).** ⚠️ 지리명 노회(서울·경기·부산 등)는 교단 간 충돌 가능 → 유일하지 않으면 ③으로.
2. **교단 직접 명시** — "예장합동", "통합측", "기독교대한성결교회" 등 → §2c alias 맵으로 확정.
3. **교회 명부 대조** — 교회명+지역을 각 총회 공식 교회검색/명부와 대조 → 유일 매칭 시 확정(정본).
4. **①~③ 없거나 모호** → **`denomination = 미상`.** (게시판 default로 찍지 않는다.) → 운영자 게이트에서 확정.

**출력 필드 (라벨 하나가 아니라 3개):**
- `denomination` — 확정 교단 key (근거 있을 때만; 없으면 `미상`)
- `denomination_source` — `nohoe` | `stated` | `registry` | `unknown`
- `denomination_evidence` — 원문에서 인용한 근거 문자열(운영자 3초 검증용)
- (참고) `denomination_hint` — 게시판 default. **확정 아님**, 운영자 참고용으로만 저장.

**AI(Gemini)의 역할 = "신호 추출"만.** 노회명·교단표기·홈페이지 URL을 raw_text에서 뽑기만 하고, **"이 노회가 어느 교단"인지 매핑은 LLM이 아니라 §2b 표/명부가** 한다(LLM 환각 방지 — 신뢰성의 핵심).

- 결과가 9개 **대형** key 밖이면 → `ETC` + 운영자 플래그(자동 게재 금지, 정통 화이트리스트).
- 항상 `raw_denomination`(원문 표기) 보존.

### 2b. 노회/연회 → 교단 매핑표 (⚠️ 초안 — 각 총회 명부로 완성 필요)
> **구조와 원칙만** 담는다. 각 교단 총회가 소속 노회 명부를 공개하므로 그걸로 **전량 채워야 정본**(별도 리서치). 검증 안 된 노회명은 임의로 넣지 않는다.

| 표현 유형 | 교단 | 처리 |
|---|---|---|
| 감리교 **"○○연회"** | GAMLI | 연회명은 감리교 확정 신호 |
| 장로교 고유 노회명(교단별 상이) | 해당 교단 | 각 총회 명부의 노회 목록과 유일 매칭 시 확정 |
| ⚠️ 지리명 노회(서울·경기·부산·경북 등) | **충돌 가능** | 여러 교단에 존재 → **단독 확정 금지**, ③ 명부 대조로 |

> **TODO(리서치)**: 합동·통합·고신·합신·기장·백석 각 총회 노회 명부 수집 → 이 표 완성. 완성 전까지는 ②명시·③명부 위주로 확정하고, 노회 신호는 "명확·비충돌"인 것만 사용.
...
**제외(크롤 안 함)**: 대전신대·아이굿뉴스·서울신대(기성 flagship `stu.ac.kr`, 게시판 미확인)·성결대·한국성결신문(게시판 없음), 예수교대한하나님의성회(휴면), 고신총회(KTS 중복). **CROSS 상업**(청빙넷·제이웹·cjob·갓피플)은 "공식 게시판만" 정책으로 초기 제외. ※ **아신대·WGST는 초교파로 편입**(아래 초교파 표) — 여기 "휴면/CROSS" 표기는 폐기.
...
| `wgst` | 웨스트민스터신대원 교역자청빙 | B | 공개 | **http 전용** · 2026 활성 재확인 후 채택(조건부) |
| `ezra` | 에스라성경대학원 초빙게시판 | B | 공개 | HTTPS · 저빈도 |
| `cts` | CTS기독교TV 채용정보 | B | **로그인**(상세) | 방송사·초교파 · 로그인 게이트 |
