...
- ⚠️ **남는 중복 두 종류**(둘 다 "안 묶는 쪽"이라 안전하다): ① 모델이 **직분 배열을 달리 뽑으면** 갈린다 — `개복교회`가 공고 문구를 `전임사역자(부목사, 강도사, 여전도사)` → `(부목사, 강도사)`로 바꾼 뒤 2묶음이 됐다(모집 범위가 실제로 바뀐 것이라 갈린 것이 옳기도 하다). ② 모델이 **교회명을 달리 뽑으면** 갈린다 — 같은 본문 13건 중 1건이 `안양동은교회` 대신 `동은교회`였다(8%). ✅ **2026-09-02에 2단계가 이 갈림을 잡는다**(아래) — 다만 **접수 메일함이 겹칠 때만**이라 메일을 안 적은 공고는 여전히 갈린다.
