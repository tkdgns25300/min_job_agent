...
- ⚠️ **부서가 섞였을 때 완화하지 않는다**(운영자 결정 2026-08-17 · 비용을 알고 내린 결정). 실측: `진천중앙교회` 19건은 본문이 같은 한 청빙인데 게시판 `모집부서` 칸이 `부목사`(5월) → `교구목사`(6월)로 바뀌어 **19건이 통째로 검수 대기**가 됐다. 그래도 두는 이유는 방향이 비대칭이기 때문이다 — 침묵을 "같은 부서"로 단정하면 부서가 다른 자리를 합쳐 **공고 하나가 사라지고 되돌릴 수 없다**. 지금까지 이 규칙이 검수로 보낸 21건 중 진짜 다른 자리는 0건이지만, 0건은 "없다"가 아니라 "아직 안 봤다"다. ⚠️ 같은 교회의 **진짜 다른 청빙은 이미 갈린다** — 진천중앙교회 7/25 공고(`부목사 2명 / 교육전도사 2명`)는 직분이 달라 별도 묶음이 됐다.
