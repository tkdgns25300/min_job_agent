...
- ⚠️ **지역이 있는 항목은 지역까지 맞아야 거절한다**(2026-08-16). 목록 원본이 `아무개(춘천 ○○교회)`처럼 지역을 적어 둔 항목이 5개 있는데, 그것을 버리면 전국의 같은 이름 교회가 전부 걸린다. **지역이 다르면 이단을 봐주는 것이 아니라 애초에 그 교회가 아니다.** 공고에 지역이 없으면 아니라고 말할 근거가 없으므로 거절한다.
- ⚠️ **이 문서에 교회·사람 실명을 적지 않는다** — 목록을 커밋하지 않는 이유(실명 자료)와 같다. 예시는 `○○교회`·`아무개`로 쓴다.
- ⚠️ **지역이 없는 항목(대부분)은 이름만 보고 거절하되, 근거에 `지역 확인 불가`를 남긴다.** 이 표시가 무고한 거절을 되짚을 유일한 실마리다 — 실측: 원장의 게시판 교회명 칸에서 14건이 걸리는데 **서로 다른 교회는 3곳**뿐이고(7·6·1건씩 끌어올림), 목록에 그 3곳의 지역이 채워지면 사라진다. ⚠️ 이 문서에 교회 실명을 적지 않는다 — 목록을 커밋하지 않는 이유와 같다.
...
- 매칭 시 **`heresy_flag=true` + `heresy_evidence`(어느 항목에 · 누가 규정 · 지역을 봤나)** + **`review_status=REJECTED` + `reject_reason=HERESY`**. ⚠️ **이단이 마감(§5.4b)보다 우선한다** — `reject_reason`이 한 칸뿐인데 마감은 그 공고의 사실이고 이단은 그 교회의 사실이다. → 검수 큐에 뜨지 않고 승격되지 않는다(운영자 결정 2026-08-06 · 앞선 "플래그만"에서 변경).
