...
4. 연락처로 가르기 ─ 이메일·전화·링크
      한 채널에서 양쪽 다 값이 있는데 다르다 → 다른 자리 (SEPARATE)
      한쪽만 있다 / 둘 다 없다               → 근거 없음 → 중복 확정
5. 대표 고르기
...
| `dedup_state` | `ALONE` · `MASTER` · `DUPLICATE` · `SEPARATE` · `UNCERTAIN` |
...
| `ALONE`·`SEPARATE` | 등급대로(`confidence`) — 지난 실행이 잘못 거절했으면 되살아난다 |
...
- ⚠️ **연락처는 가르는 데만 쓴다.** 다르면 지원할 곳이 다르니 다른 자리지만(실측 2묶음: `광림교회` 청장년부/교회학교 — 담당자·마감·이메일이 전부 달랐다), **같다고 같은 자리는 아니다**(교회 대표번호를 여러 자리에 쓴다). 그래서 3단계의 `UNCERTAIN`은 연락처가 같아도 사람에게 넘긴다. `contact_post`는 검산을 거치지 않는 조립 칸이라(§5.5b) 이 판정에 쓰지 않는다.
...
- **실측 ①**(구조화해둔 132건 · 게시판별 5건 표본): 중복 21건 제거(17묶음) · `UNCERTAIN` 2건 · `SEPARATE` 4건 · 혼자 84건 · 견줄 수 없음 2건 · 이미 결론 2건.
