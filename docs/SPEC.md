...
- **재구조화가 무엇을 되돌리나**(`is_safe_to_replace`):
  ...
  - ⚠️ **`APPROVED`는 지킨다.** 자동 승인된 행에는 고친 규칙이 적용되지 않는다: 로컬 JSON 단계에서는 `data/review_data.json`을 지우고 `scripts/reset_structure.py`로 되돌리면 되지만, Supabase 전환·데일리 이후에는 그 방법이 통하지 않는다. **둘을 가르는 표시는 `reviewed_by`가 아니어도 있다**(바로 아래) — 그 값을 쓰면 자동 승인만 되돌릴 수 있다.
