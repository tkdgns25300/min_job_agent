...
- **실측**(**1주치를 dedup·공개까지 끝까지 돌린 전량 473건** · 2026-08-23 · 2개월 환산 ×7.8): 게이트1 탈락 8 → 초안 465. 등급은 `high` 368 · `medium` 80 · `low` 17. 상태로는 **`APPROVED` 249(54% ≈ 1,950) · `PENDING` 69(15% ≈ 540) · `REJECTED` 147(31% ≈ 1,150)**이고 그중 240건이 `jobs`에 공개됐다.
  ⚠️ **이 수는 dedup 뒤의 값이다** — 거절 147건 중 **144건이 중복**(나머지 3건은 마감)이라 `PENDING` 69건은 이미 재게시를 걷어낸 뒤다(옛 표본 환산은 dedup 전이라 "42%만큼 더 줄어든다"를 따로 붙여야 했다).
  ⚠️ **사람이 볼 69건 중 61건(88%)이 그림 때문이다.** 나머지 8건은 승격 6칸이 비었거나 같은 자리 판정이 불확실한 것이다.
- ⚠️ **등급과 상태를 같은 수로 세지 않는다** — `high` 368건 중 **117건이 거절**됐다(거절이 등급보다 앞선다 · 거의 전부 중복). 거꾸로 `high`인데 `PENDING`인 2건도 있다(`dedup_state=UNCERTAIN` — 같은 자리인지 확실하지 않다). 실행 화면은 **상태**로 센다.
