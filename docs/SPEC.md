...
4. 접수 이메일로 가르기
      양쪽에 있는데 겹치지 않는다  → **주소를 본다**(4b)
      그 밖(같다·한쪽만·둘 다 없다) → 중복 확정
      ⚠️ 전화·링크·우편은 보지 않는다
4b. 주소로 가르기 ─ **메일이 갈렸을 때만**
...
- ⚠️⚠️ **그때 주소가 답을 준다**(2026-08-26 추가 · 4b). 자물쇠는 지역을 **광역까지만** 보므로 **같은 광역 안의 동명이교회를 못 가른다** — 실측: `영광교회`가 강서구(강서로 412)와 금천구(금하로 793)에, `신광교회`가 관악구와 중구에 각각 있었고 둘 다 메일이 갈려 검수로 갔다. 사람이 안 보면 **한쪽 교회의 공고가 영영 묻힌다.**

  **왜 4단계에서만 보나** — 위 "전화·링크·우편을 보지 않는다"와 모순이 아니다. 그 셋은 *"같은 자리"를 증명하는 데* 못 쓴다는 뜻이고, 여기서 쓰는 것은 반대 방향이다: **주소가 다르면 애초에 같은 교회가 아니다.** 전화가 게시판마다 흔들리는 것과 달리 **교회 주소는 하나뿐**이라 이 방향으로는 신뢰할 수 있다.
