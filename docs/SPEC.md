...
2. 이름 표기가 갈린 자리 합치기 (2026-09-02 추가)
      지역·직분이 같고 · 꼬리(`교회`)를 뗀 이름이 포함 관계이며 · **접수 메일함이 겹치면** 한 자리
      (`남광` ⊂ `광주남광` · `한밭제일` ⊂ `한밭제일장로`) · 남는 키는 **이름이 긴 쪽**
...
- ⚠️ **남는 중복 두 종류**(둘 다 "안 묶는 쪽"이라 안전하다): ① 모델이 **직분 배열을 달리 뽑으면** 갈린다 — `개복교회`가 공고 문구를 `전임사역자(부목사, 강도사, 여전도사)` → `(부목사, 강도사)`로 바꾼 뒤 2묶음이 됐다(모집 범위가 실제로 바뀐 것이라 갈린 것이 옳기도 하다).
  ...
  → ①②는 침묵·포함을 "안 적은 것"으로 읽으면 잡히고(부서 규칙과 같은 판단), ③은 손대면 안 된다. **아직 규칙을 넣지 않았다** — 운영자 결정 대기. ② 모델이 **교회명을 달리 뽑으면** 갈린다 — 같은 본문 13건 중 1건이 `안양동은교회` 대신 `동은교회`였다(8%). ✅ **2026-09-02에 2단계가 이 갈림을 잡는다**(아래) — 다만 **접수 메일함이 겹칠 때만**이라 메일을 안 적은 공고는 여전히 갈린다.
