...
⚠️ **반대로 "정말 못 받는" 경우도 있고 그건 옳은 판정이다**(2026-08-23 실측). `CSU`는 첨부를 며칠 뒤 지운다 — 수집 때 200이던 `/upload/board/…{uuid}.pdf`가 5~7일 뒤 전부 404다. 그런 공고는 그림이 원문인데 그림이 없으니 **사람이 봐야 한다**. 오래된 공고를 구조화할 때 포스터가 비는 것은 결함이 아니다. **이것이 받은 그림을 Storage에 복사해 두는 이유다** — 우리가 한 번 받아 두면 게시판이 지운 뒤에도 검수 화면이 판정을 되짚을 수 있다.
