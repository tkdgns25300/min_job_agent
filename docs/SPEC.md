...
⚠️ **"못 받았다"가 우리 쪽 결함일 수도 있다 — 원인을 확인하기 전에 등급으로 넘기지 말 것**(2026-08-23). `CSU` 첨부 3건이 "못 받음"으로 `low`가 됐는데, 게시판이 지운 것이 아니라 **우리가 다운로드 URL을 잘못 만들고 있었다**(`/upload/` ← 추측 · 정답은 `/api/file/get?path=`). 오래 드러나지 않은 이유가 중요하다: 저장된 CSU 포스터가 전부 **본문 인라인 그림**이라 포스터 수치가 멀쩡했고, **인라인은 되고 첨부만 404**인 상태를 가려 주었다. → 한 게시판에서 그림 경로가 둘이면 **둘을 따로 확인한다**(`config/sources.json` CSU `fetch_note`).

⚠️⚠️ **URL 만드는 법을 고쳐도 이미 저장된 행은 낫지 않는다.** `source_data`는 write-once(§6 ①)라 `attachments[].url`에 **옛 어댑터가 만든 주소가 그대로** 남고, 재구조화는 그 값을 읽는다 — 고친 뒤 되돌려도 같은 404가 난다(실측: CSU 3건). 원장이 `(source_key, external_id)`로 재수집도 막으므로, **고친 어댑터의 효과는 새로 수집하는 공고에만** 미친다. → 전송 경로를 고쳤으면 **그 게시판을 새로 수집할 계획인지 먼저 확인한다.**
