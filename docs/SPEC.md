...
- ⚠️ **본문·이미지·첨부만 보면 안 된다**(2026-08-22 실측으로 고쳤다). 프롬프트는 **게시판 필드(`raw_meta`)도 모델에 보낸다** — `CSU`는 교단·교회명·지역·사례비가 본문이 아니라 거기 있다. 문턱이 `SourceData.is_empty`였을 때 **CSU 1주치 125건 중 6건이 본문 없이 meta 13칸만 있었고, 전부 진짜 청빙 공고인데 "빈 입력"으로 조용히 버려졌다**(같은 비율이면 3개월에 70건이 넘는다).
