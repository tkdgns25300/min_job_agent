...
6. 대표 고르기
      사람이 확인한 것(공개됨·운영자 수정) > 자동 승인된 것 > **직분 자세히 적은 것** > 빈 칸 적은 것 > 최신 > id
```
...
- ⚠️ **대표 순위에서 직분이 빈 칸 수보다 앞이다**(운영자 결정 2026-09-02). 2단계 창구가 `기타`와 `전도사`를 한 자리로 묶은 뒤로, "빈 칸 적은 것"만으로 고르면 사례비 한 칸 더 채운 `기타` 쪽이 대표가 되어 **지원자가 직분으로 거를 때 그 자리를 못 본다.** 지원자가 거르는 칸이 앞이다. 기존 대표 460개는 전부 이미 공개된 행이라 첫 기준에서 끝나 하나도 안 움직인다 — 바뀌는 것은 새 자리가 처음 공개될 때와 백로그 정리에서 남길 공고다.
...
- ⚠️⚠️ **마감이 지난 자리에서는 살아 있는 글만 견준다**(2026-09-02 추가 · `_still_open`). 위 거절은 **구조화 때** 이미 마감이었던 글(§5.4b)만 잡는다 — 공개된 뒤 마감이 지난 대표는 `APPROVED`인 채 남아 **여전히 이기고**, 교회가 마감을 새로 달아 다시 올린 글은 **새 글**이라 §5.4b에 걸리지 않고 그 밑에 `DUPLICATE`로 묻힌다. 실측 2026-09-02: 대표는 마감으로 CLOSED, 재공고는 묻혀 **교회는 뽑는데 min_job엔 안 보이는** 자리가 3곳(까오숑·열린·선한목자). 원문 소멸(§4.4)과 **같은 길**을 연다 — 빠지면 살아 있는 재공고가 자리를 물려받아 등급대로 나간다(제일성도교회가 소멸 경로로 그렇게 된 것을 확인했다).
  ⚠️⚠️ **마감 뒤에 올라온 글만 재공고다.** 처음엔 "제 마감이 지난 글만 빠진다"로 했다가 실데이터에서 바로 고쳤다 — 형제가 있는 10자리 중 **6자리의 형제가 마감 전에 올라온 글**이었다(같은 청빙을 다른 게시판에 마감 없이 올린 교차게시). 그것을 대표로 세우면 **닫힌 청빙이 되살아난다.** 그래서 마감 없는 글은 **그 자리의 가장 늦은 마감보다 뒤에 올라왔을 때만** 산다. 제 마감이 아직 안 지난 글은 게시일과 상관없이 산다(교회가 살아 있다고 말한 글). 앵커는 늘 산다.
