...
> ⏳ **`address`만 구현됐다**(2026-08-16). `region`·`city`는 아직 `normalize.place_of`가 만든다 — 교체하면 개악일 수 있어(게시판 지역 칸 730건에서 광역 100%) **유료 표본으로 모델과 대조한 뒤** 바꾼다(ROADMAP 1-2).
...
- **코드 검산**(`pipeline/verify.py`): `region`은 **근거가 원문에 있나**까지만 본다. `city`·`address`는 원문에 그 글자가 있나(존재 검사). 없으면 **그 칸만 비운다**.
